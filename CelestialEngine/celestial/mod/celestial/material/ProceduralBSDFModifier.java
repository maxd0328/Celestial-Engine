...
public final class ProceduralBSDFModifier extends Modifier {
	
	private static final long serialVersionUID = -8793559302693107528L;
	
	public static final Factory<ProceduralBSDFModifier> FACTORY = () -> new ProceduralBSDFModifier(0f, new Vector3f(),
			1f, 1f, 0.01f, 0.2f, 0f, 0.5f, 1f, 0f, 1f, 0.1f, 2.2f, 0.04f, false, false);
	...
	public ProceduralBSDFModifier(float subsurface, Vector3f subsurfaceRadius, float subsurfaceDistortion, float subsurfaceScale, float subsurfaceThickness, float subsurfaceAmbient,
			float metallic, float roughness, float specular, float specularTint, float ambientOcclusion, float ambient, float gamma, float iorF0, boolean noCosTheta,
			boolean restrictTangents) {
	...
	private ProceduralBSDFModifier(ProceduralBSDFModifier src) {
		super(false, false, true);
	...
	protected ShaderModule getShaderModule() {
		...
		ArrayList<ShaderAttribute> fUniforms = ShaderModule.attribs($("float materialRestrictTangents"), $("float materialLightMax"),
				$("float materialMetallic"), $("float materialRoughness"), $("float materialSpecular"), $("float materialSpecularTint"),
				$("float materialAmbient"), $("float materialGamma"), $("float materialF0"), $("vec3 materialCol", LIGHT_MAX),
				...
				$("float materialSubsurfaceAmbient"), $("float materialAO"), $("float materialNoCosTheta"), $("sampler2D materialShadowMap", SHADOW_MAX),
		...
		ArrayList<ShaderAttribute> fGlobals = ShaderModule.attribs($("vec3 surfaceNorm").withDefaultValue("vec3(0, 0, 1)"), $("float materialMetallic").withDefaultValue("1.0"),
				$("float materialRoughness").withDefaultValue("1.0"), $("float materialSubsurface").withDefaultValue("1.0"), $("float materialAO").withDefaultValue("1.0"));
		return new ShaderModule("proceduralBSDF",
				new ProgramModule(Shader.getProgramSegment(getClass().getResource("material.glsl"), "proceduralBSDF_V"), vUniforms, vGlobals),
				new ProgramModule(Shader.getProgramSegment(getClass().getResource("material.glsl"), "proceduralBSDF_F"), fUniforms, fGlobals),
				getID(), ShaderModule.attribs($("vec3 normal"), $("vec3 tangent")),
				ShaderModule.attribs($("vec3 materialView"), $("vec3 materialNorm"), $("vec3 materialSAng", LIGHT_MAX),
						$("vec3 materialVDir", LIGHT_MAX), $("vec4 materialShadowCoords", SHADOW_MAX), $("float materialShadowTransition", SHADOW_MAX),
						$("float materialPointDistance"), $("vec3 materialPointDirection")));
	}
	...
	public Modifier duplicate() {
		return new ProceduralBSDFModifier(this);
	}
