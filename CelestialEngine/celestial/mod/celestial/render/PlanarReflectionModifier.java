...
public final class PlanarReflectionModifier extends AbstractSamplerModifier {
	
	private static final long serialVersionUID = 2744216650876114242L;
	
	public static final Factory<PlanarReflectionModifier> FACTORY = () -> new PlanarReflectionModifier(320, 180, new Vector3f(0, 1, 0), 1f, false, 1f, false, 0);
	
	private static final TreeMap<Float, KVEntry<FrameBuffer, CEObject>> FRAMEBUFFER_MAP = new TreeMap<Float, KVEntry<FrameBuffer, CEObject>>();
	...
	public PlanarReflectionModifier(int width, int height, Vector3f normal, float blendFactor,
			boolean fresnelBlending, float fresnelModifier, boolean distortEnabled, int distortUnit) {
		super(false, true, "planarReflection", 1, FrameBuffer.create(width, height, false, true, ColorDepth.RGBA8_LDR), 0, distortEnabled, distortUnit);
	...
	private PlanarReflectionModifier(PlanarReflectionModifier src, int width, int height) {
		super(src, FrameBuffer.create(width, height, false, true, ColorDepth.RGBA8_LDR));
	...
	protected ShaderModule getShaderModule() {
		...
		ArrayList<ShaderAttribute> fUniforms = fUniforms(ShaderModule.attribs(ShaderAttribute.$("vec3 planarReflectionBlend"),
				ShaderAttribute.$("vec3 planarReflectionNormal")));
		ArrayList<ShaderAttribute> fGlobals = ShaderModule.attribs();
		return new ShaderModule("planarReflection",
				new ProgramModule(Shader.getProgramSegment(getClass().getResource("render.glsl"), "planarV"), vUniforms, vGlobals),
				new ProgramModule(Shader.getProgramSegment(getClass().getResource("render.glsl"), "planarReflectionF"), fUniforms, fGlobals),
	...
	protected void preRender(RenderPacket packet, CEObject obj) {
		...
		Vector3f normal = this.normal.get().clone().normalize();
		new Matrix3f(bufObj.getRotationTransformation()).transform(normal, normal);
		
		if(!frameRendered) {
			frameRendered = true;
			float planeConstant = normal.x * bufObj.getPosition().x + normal.y * bufObj.getPosition().y + normal.z * bufObj.getPosition().z;
			Vector4f plane = new Vector4f(normal.x, normal.y, normal.z, -planeConstant + 0.01f);
			
			packet.getCamera().getModifier(CameraModifier.class).reflect(plane, packet.getCamera());
			...
			packet.getCamera().getModifier(CameraModifier.class).reflect(null, packet.getCamera());
		...
		packet.getShader().getCommunicator().store3f("planarReflectionBlend", new Vector3f(blendFactor.get(), fresnelBlending.get() ? 1f : 0f, fresnelModifier.get()));
		packet.getShader().getCommunicator().store3f("planarReflectionNormal", normal);
	}
	...
	public Modifier duplicate() {
		return new PlanarReflectionModifier(this, buffer.getWidth(), buffer.getHeight());
	}
	
	private Modifier duplicate(int width, int height) {
		return new PlanarReflectionModifier(this, width, height);
	}
	
	@SerializerImpl
	private void readObject(ObjectInputStream in) throws ClassNotFoundException, IOException {
		in.defaultReadObject();
		super.loadImplementation("planarReflection", 1);
	}
