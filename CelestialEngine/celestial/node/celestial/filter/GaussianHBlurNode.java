...
public final class GaussianHBlurNode extends ShadingNode {
	
	private static final Shader SHADER = new Shader(
			Shader.getProgramSegment(GaussianHBlurNode.class.getResource("filter.glsl"), "gaussianHBlurV"),
			Shader.getProgramSegment(GaussianHBlurNode.class.getResource("filter.glsl"), "gaussianBlurF"),
			Arrays.asList(ShaderAttribute.$("vec2 position")),
			Arrays.asList(ShaderAttribute.$("float kernelRadius"), ShaderAttribute.$("float sigma"), ShaderAttribute.$("float targetWidth"), ShaderAttribute.$("sampler2D color"))
	);
	...
	public GaussianHBlurNode(int kernelRadius, float sigma, float spread) {
	...
	public void preRender(Shader shader, ShadingSystem system) {
		...
		shader.getCommunicator().store1f("targetWidth", system.getTexWidth() / spread.get());
	}
