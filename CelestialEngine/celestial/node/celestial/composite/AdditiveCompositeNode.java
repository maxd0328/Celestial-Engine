...
public final class AdditiveCompositeNode extends ShadingNode {
	
	private static final Shader SHADER = new Shader(
			Shader.getProgramSegment(AdditiveCompositeNode.class.getResource("composite.glsl"), "compositeV"),
			Shader.getProgramSegment(AdditiveCompositeNode.class.getResource("composite.glsl"), "additiveCompositeF"),
			Arrays.asList(ShaderAttribute.$("vec2 position")),
			Arrays.asList(ShaderAttribute.$("sampler2D bottom"), ShaderAttribute.$("sampler2D top"))
	);
	
	public AdditiveCompositeNode() {
